...
type SuccessfulStatusCode = typeof StatusOk | typeof StatusCreated | typeof StatusNoContent
...
export type ApiResponse<TData> = SuccessfulApiResponse<TData> | ErrorApiResponse
...
const isSuccessfulStatusCode = (statusCode: StatusCode): statusCode is SuccessfulStatusCode => {
    return [StatusOk, StatusCreated, StatusNoContent].includes(statusCode)
}

const isErrorStatusCode = (statusCode: StatusCode): statusCode is ErrorStatusCode => {
    return !isSuccessfulStatusCode(statusCode)
}

export function newApiResponse(statusCode: ErrorStatusCode, message: StringOrError): ErrorApiResponse
export function newApiResponse<TData>(statusCode: SuccessfulStatusCode, data: TData): SuccessfulApiResponse<TData>

export function newApiResponse<TData>(statusCode: StatusCode, data: TData): ApiResponse<TData> {
